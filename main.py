...
if __name__ == "__main__":
    ...
    cli.questions.update({"name": ["the habit name", "Any text is valid up to 20 letters"],
                          ...
                          "time": ["a time value", "Any number up to 1440 is valid. "
                                                   "This is optional,\nif you want to skip this enter 0"],
    ...
    message_error: str = "An unknown error occurred. Please copy the previous output and send it to developer :)"
    ...
    def create_habit() -> None:
        ...
        if not habit.is_existing(habit.name):
            ...
            if create_status:
                cli.helper_clear_terminal()
                print(
                    "Successfully created habit \"{name}\" with the description \"{description}\" and a \"{periodicity}\""
                    " periodicity.\nThe first time it needs to be checked is until the end of the "
                    "\"{next_periodicity_due_date}\" and \"{default_time}\" minutes are added by default."
            ...
            print("A habit with the name \"{name}\" already exists! Please choose another name!"
                  .format(name=habit.name))
        cli.helper_wait_for_key()


    def update_habit() -> None:
        cli.helper_clear_terminal()
    ...
    cli.main_menu_options.update({1: "Create a habit",
                                  2: "Update a habit",
                                  3: "Analyse habits",
                                  4: "Delete a habit",
    ...
    def analyse_habit_longest_streak():
        name = cli.validate("name", "name")
        if habit.is_existing(name):
            habit.set_id(name)
    ...
    def analyse_all_habits_longest_streak():
    ...
    def analyse_habits_same_periodicity():
        periodicity: str = cli.validate("periodicity", "periodicity")
        habit.periodicity = helper_type_conversions(periodicity)
    ...
    def analyze_habits(option: str) -> None:
        """
    ...
    submenu_analyse_options: dict = {1: "Show all currently tracked habits",
    ...
    submenu_analyse_functions: dict = {1: lambda: analyze_habits("all"),
                                       2: lambda: analyze_habits("all same periodicity"),
                                       3: lambda: analyze_habits("longest streak of all"),
                                       4: lambda: analyze_habits("longest streak"),
                                       5: lambda: analyze_habits("time"),
                                       8: lambda: cli.menu()}
    ...
    def helper_format_and_output(result: list) -> None:
        ...
        for i in result:
            name = i[1]
            description = i[2]
            periodicity = helper_type_conversions(i[3])
