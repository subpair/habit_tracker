...
class TestDatabase:
    ...
    def setup_method(self) -> None:
        ...
        self.date_today: date = datetime.strptime("2022-01-01", self.date_format).date()
        self.next_periodicity_due_date: date = datetime.strptime("2022-01-02", self.date_format).date()
        self.dummy_name: str = "new habit"

        create_habit_status: bool = self.database.create_new_habit(self.dummy_name, "a new one", 1, self.date_today,
                                                                   self.next_periodicity_due_date, 0)
        assert create_habit_status is True

        create_event_status: bool = self.database.create_new_event(1, True, self.date_today, 0,
    ...
    def test_read_next_periodicity_due_date(self) -> None:
        ...
        assert self.database.read_next_periodicity_due_date(1)[0] == "2022-01-02"
    ...
    def test_update_next_periodicity_due_date(self) -> None:
        ...
        next_periodicity_due_date: date = datetime.strptime("2022-01-03", self.date_format).date()
    ...
    def test_update_default_time(self) -> None:
        """Test updating default time value of an existing record."""
        default_time: int = 45
        assert self.database.update_default_time(1, default_time) is True
        assert self.database.read_habit_default_time(1)[0] == 45

    def test_read_habits_events_change_id(self) -> None:
        """Test reading the change id of an existing event record."""
        change_id: int = self.database.read_habits_events_change_id(1, self.next_periodicity_due_date)[0]
        assert change_id == 1

    def test_read_event_record(self) -> None:
        """Test reading a single event record from the database."""
        result: tuple = self.database.read_all_habits_event_records(1, self.next_periodicity_due_date)
        assert result == (1, 1, 1, 0, str(self.date_today), str(self.next_periodicity_due_date))

        # single event record
        result = self.database.read_habit_event_record(1)
        assert result == (1, 1, 1, 0, str(self.date_today), str(self.next_periodicity_due_date))

    def test_update_habits_event_completion(self) -> None:
        """Test updating the completion value of an existing event record."""
        new_completed: bool = False
        assert self.database.update_habits_event_completion(1, new_completed, self.date_today) is True
        assert bool(self.database.read_all_habits_event_records(1, self.next_periodicity_due_date)[2]) == new_completed

    def test_update_habits_event_time(self) -> None:
        """Test updating the time value of an existing event record."""
        new_time: int = 30
        assert self.database.update_habits_event_time(1, new_time, self.date_today) is True
        assert self.database.read_all_habits_event_records(1, self.next_periodicity_due_date)[3] == new_time
