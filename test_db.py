...
class Tests:
    ...
    def setup_method(self) -> None:
        ...
        insert_habit_status = self.database.create_habit(self.dummy_name, "a new one", 1, self.date_today,
                                                         self.next_periodicity_due_date, 0)
        assert insert_habit_status is True

        insert_event_status = self.database.create_event(1, True, self.date_today, 0)
        assert insert_event_status is True

    def test_insert_habit(self) -> None:
        insert_habit_status = self.database.create_habit("another habit", "a newer one", 1, self.date_today,
                                                         self.next_periodicity_due_date, 0)
        assert insert_habit_status is True
    ...
    def test_analyse_get_all(self) -> None:
        assert len(self.database.read_habits_by_finished()) >= 1
